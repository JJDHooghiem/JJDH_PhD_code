...
def main():
    ...
    xlabs = [config.axl['co'], config.axl['co2']]
    ylabs = [config.axl['alt']]*2
    xlims = [(20, 120), (401, 406)]
    ylims = [(10, 15)]*2
    ...
    for dat, c, l in zip(AC_keys, colors, labels):
        ...
        axes_array[0].axhline(tropopause, linestyle=':', color=c, alpha=0.7)
        # x=data[dat]['P']
        # y=data[dat]['CO2'] 
        ...
        mask,x=scantools.asym_convolve(x,y,2,return_mask=True)
        y=data[dat]['GPS_ALT'][mask]/1000
        axes_array[0].plot(x, y, color=c, label=l, alpha=0.7)
    ...
    for dat, m, l, c in zip(LISA_keys, markers, labels, colors[1:]):
        ...
        axes_array[0].errorbar(LISA[dat]['CO'], LISA[dat]['Altitude']/1000,
    ...
    fig.tight_layout()
    leg = axes_array[0].legend(ncol=3, loc='lower center', bbox_to_anchor=(
        0.5, -0.09), bbox_transform=fig.transFigure)
    fig.savefig(config.FigDir+'wisman_molefraction.pdf',
