...
for dat, m, c in zip(lodautil.get_dates(lisa), markers, colors):
    ...
    for ax, xkey in zip(axes.ravel(), ['CO', 'CH4', 'd18O(CO)', 'd13C(CO)']):
        ax.plot(data[xkey], data[ykey], marker=m, linestyle='-', color=c, label=dat)
axes[0, 0].legend()
fig.tight_layout()
fig.savefig(config.FigDir+'stico_lisa_tracers.pdf')
...
for dat, m, c in zip(lodautil.get_dates(lisa), markers, colors):
    ...
    ykey = 'Altitude'
    for ax, xkey in zip(axes.ravel(), ['CO', 'CH4', 'd18O(CO)', 'd13C(CO)']):
        ax.plot(data[xkey], data[ykey]/1000, marker=m, linestyle='-', color=c, label=dat)
axes[0, 0].legend()
fig.tight_layout()
fig.savefig(config.FigDir+'stico_lisa_tracers_alt.pdf')
