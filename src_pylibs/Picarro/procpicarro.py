...
Radiosonde,Datalogger,sample_info,LISAdata,figw,ofsets=process_met_data(datalogger_file,radiosonde_file)
...
if 'Processed/recal_file.json' not in f: 
    ...
    while more_calibration=='y':
        cal_list.extend(select_data(data['index'], data['CO2_cor'],'for calibration'))
...
#    key=str(input('which species to overwrite?\n')) 
    ...
    os.chdir('Figures')
    plot_picarro_data(data,pic_samples2,fname='Recalc_'+key,title='Recalculation for '+key)
    os.chdir('..')
else:
    add_info=''
    pass
...
for iterkey in Datalogger.keys():
    if iterkey!='Date':
        ...
        plt.xlabel('Time')
        plt.ylabel(iterkey)
        plt.savefig('Datalogger_'+iterkey+'.pdf',format='pdf',bbox_inches='tight')
        plt.close()
...
with open('Processed/{}_LISA_flight.csv'.format(date.replace('-','')),'w') as result:
    ...
    result.close()
Datalogger.to_csv('Processed/{}_LISA_metdata.csv'.format(date.replace('-','')))
