...
import pandas as pd
#%%
#to open a dat file with pandas: data.to_pickle and pd.read_pickle(file_name)
#data=pd.read_csv('/Users/Joram/Documents/Trial data/CFKBDS2102-20170208-151435Z-DataLog_User.dat',sep='\s+',dtype=None)   
...
#%%
def covcor(data):
    a,b,c,d=2.306,4.405,-2.093,0.315
    data["deltaCO"]= a*data["b_h2o_pct"] + b*data["b_h2o_pct"]**2 + c*data["b_h2o_pct"]**3 + d*data["b_h2o_pct"]**4
    COcor=(data["CO_raw"]-data["deltaCO"])/(1-0.0124*data['h2o_reported']-0.0006*data['h2o_reported']**2)
    return COcor
#%%
    
def co2cal(data,drift=0):
    data['CO2_uncal']=data['CO2_cor']
    CO2cor=((data['CO2_uncal']*1.005673599)+0.434573159)+drift
    return CO2cor
    
def ch4cal(data,drift=0):
    data['CH4_uncal']=data['CH4_cor']
    CH4cor=((data['CH4_uncal']*0.99913183)+3.335009436)+drift
    return CH4cor 

def cocal(data,drift=0):
    data['CO_uncal']=data['CO_cor']
    COcor=((data['CO_uncal']*1.00382352365)+9.82758190299)+drift
    return COcor 
#%%
...
#%%
  #%%
def datacorrectuncal(data,pcutoff,tcutoff):
    data=data
    removefill(data)
    time_stamp(data)
    cavitycutoff(pcutoff,tcutoff,data)
