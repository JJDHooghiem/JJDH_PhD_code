...
float prsT, prsP, sumP = 0.0, avgP = 99999, P0 = 0.0, dP;
...
void setupDatalogger()
{
  do 
    {
      getMode();  
      switch(serialReadString().toInt()) { 
      case 1 : setClockofDatalogger(); break;  // SET THE TIME AND DATE OF THE CLOCK OF THE DATALOGGER
      case 2 : getDatalogFileName();   break;  // GET THE FILENAME FOR SAVING THE DATA TO THE DATALOGGER
      case 3 : checkClock();           break;  // CHECK THE TIME AND DATE OF THE CLOCK OF THE DATALOGGER
      case 4 : checkDatalogFileName(); break;  // CHECK THE AUTOMATICVALVE OF THE DATALOGGER
      case 5 : checkTemperature();     break;  // CHECK THE AUTOMATICVALVE OF THE DATALOGGER
      case 6 : checkPressure();        break;  // CHECK THE AUTOMATICVALVE OF THE DATALOGGER
      case 7 : checkAutomaticValve();  break;  // CHECK THE AUTOMATICVALVE OF THE DATALOGGER
      case 9 : mode = true;                    // START THE MEASUREMENT. 
...
int SetTimeDate(int d, int mo, int y, int h, int mi, int s) { 
  ...
  Serial.print("SetTimeDate ");
  Serial.println(TimeDate[0]);  Serial.println(TimeDate[1]);  Serial.println(TimeDate[2]);  Serial.println(TimeDate[3]);  Serial.println(TimeDate[4]);
  Serial.println(TimeDate[5]);  Serial.println(TimeDate[6]); 
  for(int i=0; i<=6;i++){
    if(i==3) i++;
    int b= TimeDate[i]/10;
...
void getMode(){
  ...
  Serial.println("<2> Get FileName: ");
  Serial.println("<3> Check time. ");
  Serial.println("<4> Check Filename. ");
  Serial.println("<5> Check Temperature. ");
  Serial.println("<6> Check Pressure. ");
  Serial.println("<7> Check AutomaticValve. ");
  Serial.println("<0> Quit setup, start logger. ");
}
     
void setClockofDatalogger(){
...
void findNewFileName()
{
  ...
  {
    sprintf(charFileName, "%02d%02d%02d%02d.csv", year, month, day, i);
  ...
  if (debug) Serial.println(charFileName);
