...
//  byte year, month, date, hour, minute, sec;
  ...
  if (mode)
  {
    if (mode != previousMode)
    {
      ...
      dataFile.println("Date, Pressure, Temperature, T0, T1, T2, PULSE");
      dataFile.close();
...
// mdv    rtc.getDateStr(year, month, date);
// mdv    rtc.getTimeStr();
// mdv    year = 15; month = 06; date = 19; hour = 18; minute = 05; sec = 10;
// mdv    datiStr =timeString2(year, month, date, hour, minute, sec);
...
*/
/* Read digital pin for flight position */

    buttonState = digitalRead(SelectGPST);
