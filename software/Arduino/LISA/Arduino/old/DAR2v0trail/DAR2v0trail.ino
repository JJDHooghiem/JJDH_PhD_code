...
int cutterON = 300, delayPump = 1000; 
...
/*
  * SPI bus as follows:
 ** MOSI - pin 11 on Arduino Mega
 ** MISO - pin 12 on Arduino Mega
 ** CLK  - pin 13 on Arduino Mega
 ** SD   - pin 10 SD card. 
 ** RTC  - pin  9 RTC
 ** PRS -  pin  8 Pressure sensor. 
 ** ADC -  pin  7 Temperature ADS1248 with 3x PT100 
 ** ADC -  pin  6 Temperature ADS1248 with 3x PT100 
 * SelectPack0 = 3;  // Valve Pack unit 0
 * SelectPack1 = 5;  // Valve Pack unit 1 A
 * SelectPack2 = 17;  // Valve Pack unit 2
 * SelectPack3 = 16;  // Valve Pack unit 3 
 * SelectPack4 = 15;  // Valve Pack unit 3 
 * SelectPump  = 4;   // Pump unit
...
void setupServo(){
  ...
  delay(3500);             
  myservo.attach(SelectPack0);      // attaches the servo on pin 9 to the servo object 
  myservo.write(ClosePack);        // sets the servo position according to the scaled value 
  delay(700);             
  myservo.detach();               // attaches the servo on pin 9 to the servo object 
  myservo.attach(SelectPack1);      // attaches the servo on pin 9 to the servo object 
  myservo.write(OpenPack);        // sets the servo position according to the scaled value 
  delay(delayPump);
  myservo.write(ClosePack);        // sets the servo position according to the scaled value 
  delay(700);
  myservo.detach();               // attaches the servo on pin 9 to the servo object 
...
void loop(void)
{
  ...
  if (mode)
  {
    if (mode != previousMode)
    {
      ...
      dataFile.println("Date, Pressure, Temperature, Status0, Status1, T0, T1, T2, T3, T4, T5, T6, Switch");
      dataFile.close();
