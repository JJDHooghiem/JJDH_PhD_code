...
const int SelectActiveMode = 8;  // Valve Pack unit 3 
...
void setup(void)
{
  ...
  pinMode(SelectActiveMode, OUTPUT);
  ...
  digitalWrite(SelectActiveMode, LOW);
...
void setupServo()
{
  ...
  digitalWrite(SelectActiveMode, HIGH);   // turn the LED on (HIGH is the voltage level)
  
  Serial.println("Pumpy");
  delay(delayPump);
  digitalWrite(SelectPump, LOW);
  digitalWrite(SelectActiveMode, LOW);   // turn the LED on (HIGH is the voltage level)
    ...
    if (!mode)
    {
      ...
      else if (serialString == "PULSE")
      {
        ...
        if (!mode)
        {
           ...
           digitalWrite(SelectActiveMode, LOW);    // turn the LED off by making the voltage LOW
           delay(20000);                       // wait for a second
           digitalWrite(SelectActiveMode, HIGH);   // turn the LED on (HIGH is the voltage level)
           Serial.println("Pulse high");
           delay(20000);                       // wait for a second
