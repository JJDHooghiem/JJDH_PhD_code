...
const int SelectPRS  = 48;//45;   // Pressure Sensor
...
void Init_Servo()
{
  ...
  for(int SelectServo=3; SelectServo<=7; SelectServo++)
  {
...
/* Begin Pump and Servo's */


/* End Pump and Servo's */
...
/*   END GPS */
    ...
    SPIsettingsCOM();
    Serial.print("pumpON : "); // display tekst to scherm. 
    Serial.print(pumpON); // display tekst to scherm. 
    Serial.print("prsP : "); // display tekst to scherm. 
    Serial.println(prsP); // display tekst to scherm. 
...
/* ******* Measssurement Package ********************* */
    ...
    { //start-up
      OutServo.attach(SelectPack0);        // attaches the servo on pin 9 to the servo object 
      PackServo.attach(SelectServo);       // attaches the servo on pin 9 to the servo object 
      Serial.print("attach servo    \|/         "); // display tekst to scherm. 
    ...
    { // start pump
      OutServo.write(OpenPack);           // sets the servo position according to the scaled value 
      PackServo.write(ClosePack);           // sets the servo position according to the scaled value 
      Serial.print("Open/close servo    <<***>>         "); // display tekst to scherm. 
      Serial.println(SelectServo); // display tekst to scherm. 
    }
    ...
    { // start pump
      ...
      PackServo.detach();               // attaches the servo on pin 9 to the servo object   
      digitalWrite(SelectPump, HIGH);
      Serial.println("detach Servo and Start Pumpy"); // display tekst to scherm. 
      }  
    ...
    { // servo package open
      PackServo.write(OpenPack);           // sets the servo position according to the scaled value 
      OutServo.write(ClosePack);           // sets the servo position according to the scaled value 
      Serial.println("Open Package <<**>> "); // display tekst to scherm. 
    }  
    else if (pumpON == (delayPump-13)) 
    { // servo package open
      OutServo.detach();                // attaches the servo on pin 9 to the servo object 
      PackServo.detach();               // attaches the servo on pin 9 to the servo object   
      Serial.println("detach servo <<**>> "); // display tekst to scherm. 
    }  
...
/* ***************************************** */    
    ...
    {
      PackServo.attach(SelectServo);       // attaches the servo on pin 9 to the servo object 
      Serial.println("attach servo <<**>> "); // display tekst to scherm. 
    }  
    else if (pumpON == 5) 
    {
      PackServo.write(ClosePack);        // sets the servo position according to the scaled value
      Serial.println("close Package <<**>> "); // display tekst to scherm. 
    }  
    else if (pumpON == 4) 
    {
      digitalWrite(SelectPump, LOW);
    ...
    {
      OutServo.write(OpenPack);          // sets the servo position according to the scaled value 
      Serial.println("open output  <<**>> "); // display tekst to scherm. 
    }  
    else if (pumpON == 2) 
    {
      OutServo.detach();                // attaches the servo on pin 9 to the servo object 
      Serial.println("detach Output <<**>> "); // display tekst to scherm. 
    }  
    else { }
