...
int cutterON = 2000, delayPump = 1000; 
...
void loop(void)
{
  ...
  if (mode) {
    ...
    getPressure(prsP2, prsT2, 0);

    
    dtostrf((prsP+prsP2)/2, 1, 2, prsPChar);
    ...
    {
      OutServo.detach();               // attaches the servo on pin 9 to the servo object 
      PackServo.detach();               // attaches the servo on pin 9 to the servo objec
    } 
    if (cutterON == 10) 
    {
      digitalWrite(SelectPump, LOW);
